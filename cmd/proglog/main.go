...
// setupConfig reads the configuration and prepares the agent's config. Cobra calls setupConfig() before running the command's RunE.
func (c *cli) setupConfig(cmd *cobra.Command, ars []string) error {
	var err error
...
// run creates the agent, handles signals from the os, and shuts down the agent gracefully when os terminates program.
