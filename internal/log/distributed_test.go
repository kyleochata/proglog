...
// TestMultipleNodes creates a three-server cluster and shortens the default Raft timeout configs so that Raft can elect the leader quickly
func TestMulitpleNodes(t *testing.T) {
	...
	//check for replication by appending records to the lead server
	//uses eventually to give Raft enough time to replicate to all other servers in the cluster.
	...
	//GetServers returns all 3 servers in the cluster and sets the leader server as the leader
	...
	//check that the leader stops replicating to a server that's left the cluster, while continuing on remaining servers.
	err = logs[0].Leave("1")
	require.NoError(t, err)
